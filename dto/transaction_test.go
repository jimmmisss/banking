...
func Test_should_return_error_when_is_not_is_transaction_type_withdrawal(t *testing.T) {
	// Arrange
	request := TransactionRequest{
		TransactionType: "invalid transaction type",
	}
	...
	// Act
	b := request.IsTransactionTypeWithdrawal()

	// Assert
	if b {
		t.Error("Invalid transaction type")
	}
}
